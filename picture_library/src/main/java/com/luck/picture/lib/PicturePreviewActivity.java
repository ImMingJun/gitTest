        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            if (media != null) {
                ...
                if (isGif && !media.isCompressed()) {
                    PictrureMediaLoader.getInstance().displayRaw(imageView, path, 400, 800, true, null);
                } else {
                    PictrureMediaLoader.getInstance().displayRaw(imageView, path, 400, 800, false, null);
                }
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case UCrop.REQUEST_CROP:
                    ...
                    setResult(RESULT_OK, new Intent().putExtra(PictureConfig.EXTRA_OUTPUT_URI,
                            resultUri));
                    finish();
                    break;
            }
