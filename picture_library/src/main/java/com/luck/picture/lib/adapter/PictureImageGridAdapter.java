...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public PictureImageGridAdapter(Context context, PictureSelectionConfig config) {
        ...
        this.isAllSelection = config.isAllSelection;
        ...
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
            ...
            if (mimeType == PictureMimeType.ofAudio()) {
                ...
                if (overrideWidth <= 0 && overrideHeight <= 0) {
                    PictrureMediaLoader.getInstance().displayThumbnail(contentHolder.iv_picture, path, 50, 50);
                } else {
                    PictrureMediaLoader.getInstance().displayThumbnail(contentHolder.iv_picture, path, overrideWidth, overrideHeight);
                }
            }
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (!TextUtils.isEmpty(pictureType) && !isAllSelection) {
        ...
        if (selectImages.size() >= maxSelectNum && !isChecked) {
            String str;
            if (isAllSelection)
                str = context.getString(R.string.picture_message_pv_max_num, maxSelectNum);
            else {
