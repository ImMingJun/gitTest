...
public class ListVideoUtil {
    ...
    private static boolean isSmall; //当前是否小屏
    ...
    private ArrayList<Integer> logPosition = new ArrayList<>();
    ...
    public static ListVideoUtil getInstance(Context context){
        if(null == listVideoUtil){
    ...
    public void startPlay(String url) {
        ...
        gsyVideoPlayer.setUp(url, false, cachePath, mapHeadData, objects);//true,m3u8的直播流有问题。
        ...
        gsyVideoPlayer.startPlay();
    }

    public void setStartImg(int startId){
        gsyVideoPlayer.setStartImg(startId);
    }

    public void setPauseImg(int pauseId){
        gsyVideoPlayer.setPauseImg(pauseId);
    }
    ...
    private void resolveFullAdd() {
        fullViewContainer.setBackgroundColor(context.getResources().getColor(R.color.transparent));
    ...
    private void resolveMaterialAnimation() {
        ...
        lp.setMargins(listItemRect[0], listItemRect[1]+titleBarHeight, 0, 0);
    ...
    private class DrawComplete implements ViewTreeObserver.OnGlobalLayoutListener{
        ...
        public void onGlobalLayout() {
            ...
            if(null != childView){
                childView.getViewTreeObserver().removeOnGlobalLayoutListener(drawableComplate);
            }
        }
    }

    private View childView;
    DrawComplete drawableComplate = new DrawComplete();
    /**
     * 恢复小屏幕效果
     */
    private void smallVideoToNormal() {
        isSmall = false;
        gsyVideoPlayer.hideSmallVideo();
    }

    public void smallVToNormal(final View childView){
        isSmall = false;
        this.childView = childView;
        if(null != childView){
            childView.getViewTreeObserver().addOnGlobalLayoutListener(drawableComplate);
        }else{
            smallVideoToNormal();
        }
    }
     ...
     * 设置全屏显示的viewGroup
     *
     * @param viewId viewGroup
     */
    public void setFullViewContainer(int  viewId) {
        if(null == fullViewContainer)
            this.fullViewContainer = (ViewGroup) ((Activity)(context)).getWindow().getDecorView().findViewById(viewId);
    }
     ...
     */
    public void setVideoAllCallBack(final StandardVideoAllCallBack videoAllCallBack) {
